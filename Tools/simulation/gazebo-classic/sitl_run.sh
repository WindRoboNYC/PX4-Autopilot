...
if [ -x "$(command -v gazebo)" ]; then
	...
	while gz model --verbose --spawn-file="${modelpath}/${model}/${model_name}.sdf" --model-name=${model} -x 0.0 -y 0.0 -z 0.53 2>&1 | grep -q "An instance of Gazebo is not running."; do
